...
describe("validateCondition", () => {
  ...
  it("validates conditions with logical operators", () => {
    const result = validateCondition(
      "{altitude} greater_than 10000 and {ias} less_than 250",
    );
    expect(result.isValid).toBe(true);
    expect(result.errors).toHaveLength(0);
  });
  ...
  it("rejects conditions without variables", () => {
    ...
    expect(result.errors).toContain(
      "Condition must contain at least one variable in curly braces",
    );
  });
  ...
  it("rejects conditions without operators", () => {
    ...
    expect(result.isValid).toBe(false);
    expect(result.errors).toContain(
      "Condition must contain a valid comparison operator",
    );
  });
});
...
describe("validateMessage", () => {
  ...
  it("rejects messages with unknown variables", () => {
    ...
    expect(result.errors[0]).toContain(
      "Unknown variable in message: {unknownVar}",
    );
  });
});
