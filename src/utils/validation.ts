...
const COMPARISON_OPERATORS: ComparisonOperator[] = [
  ...
  'increased_by', 'decreased_by', 'changed', 'increased', 'decreased'
];

const ALL_VARIABLES = Object.values(VARIABLES).flatMap(category => category.map(v => v.value));
...
export function validateCondition(condition: string): ValidationResult {
  ...
  const hasValidOperator = COMPARISON_OPERATORS.some(op => condition.includes(op));
  if (!hasValidOperator) {
  ...
  parts.forEach((part, index) => {
    if (index % 2 === 0) { // Should be a condition segment
